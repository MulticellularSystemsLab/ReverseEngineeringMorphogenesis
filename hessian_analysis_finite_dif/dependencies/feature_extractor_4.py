...
#		plt.show()
	...
	def tissue_efd_coeff(self, harmonic):
		...
		# # Extracting the nodes located in the basal surface of tissue (both squamous and columnar)
			...
			contour_apical_x = self.vpos_x[0:self.n_total-1]
			contour_apical_y = self.vpos_y[0:self.n_total-1]
			
			# Using spatial efd library to extract efd coefficients from tissue boundary points
			coeffs_basal = spatial_efd.CalculateEFD(contour_basal_x, contour_basal_y, harmonic)
			# Normalizing the coefficients against rotation and size
			coeffs_norm_basal, rotation_basal = spatial_efd.normalize_efd(coeffs_basal, size_invariant=True)
			
			coeffs_apical = spatial_efd.CalculateEFD(contour_apical_x, contour_apical_y, harmonic)
			coeffs_norm_apical, rotation_apical = spatial_efd.normalize_efd(coeffs_apical, size_invariant=True)
...
#		plt.show()
		...
		return coeffs_basal, coeffs_norm_basal, rotation_basal, coeffs_apical, coeffs_norm_apical, rotation_apical
